...
| Asset | Spec | Status |
|---|---|---|
| App icon source (cap:assets) | 1024² ×3 | ✅ **`assets/icon-only.png` + `icon-foreground.png` + `icon-background.png`** (Claude-Design export; turret in the adaptive safe zone). `cap:assets` generates every iOS + Android density from these. |
| Launcher / adaptive icon | generated by `npm run cap:assets` | ✅ unblocked — runs off the sources above (after `cap add`) |
| Store-listing icon | **512×512** PNG | ✅ **`docs/store-assets/play-store-icon-512.png`** — *separate* Play Console upload (not generated by cap:assets). It's full-bleed so its alpha channel is harmless; flatten only if Play objects. |
| Native splash source | 2732² (`assets/splash.png` [+ `splash-dark.png`]) | ⬜ **optional** — without it the native splash falls back to the dark `backgroundColor` (#0a0f1c), which is fine since we hide it on boot. A branded splash is polish; have Claude-Design export one the same way as the icon. |
| Feature graphic (Play) | **1024×500** | ✅ **`docs/store-assets/feature-graphic-1024x500.png`** — turret scene + "MECH TIDE" title, brand fonts, cyan/gold accent (headless-Chrome render). |
| Screenshots | phone (+ tablet/iPad); iOS device sizes | ✅ **`docs/store-assets/screenshots/`** — 8 full-bleed shots (home/battle/skill-tree/upgrade-shop × portrait 1080×1920 + landscape 1920×1080). Captured from the live web build via headless Chrome at emulated phone DPR (accepted aspect; modern 20:9 device captures exceed Play's 2:1 limit). Pick 2–8 for the listing. |
| Short + full description | 80 / 4000 char | ✅ **`docs/store-assets/listing-copy.md`** — "hold the center against robots from every side" angle; no `$`, analytics-only honest. Pending Shannon's wording approval before it goes live. |

> **Note:** `@capacitor/assets` v3 expects the flat icon as **`icon-only.png`** (not
> `icon.png` — that's the v2 name the export README used). Already renamed on copy.
...
1. ~~App icon source~~ ✅ done — `assets/` has the three 1024² sources; Play 512 in `docs/store-assets/`.
